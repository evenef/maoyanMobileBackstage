	...
	methods:{
		...
		add_roomFuc(){
			...
			this.addRoomVisible = false	
			  console.log(this)
		      console.log(this.Id)
		   axios.get("http://localhost:3000/rooms/add", {
			params: {
		...
		async showRoomFuc(index,row) {
			...
			console.log(11)
				...
				const {data} = await axios.get("http://localhost:3000/rooms/find", {
					params: {
		...
		async delplayRoomFuc(index,row){
			...
			console.log(row)
	       const {data}  = await axios.get('http://localhost:3000/rooms/del',{
	        	params:{
	        		_id:row._id
	        	}
		...
		resetFunSubmit(){
			...
			axios.get("http://localhost:3000/rooms/update", {
				params: {
